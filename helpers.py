...
def create_node(action, map, goal, parent):     
    ...
    return {"state":action, 'f':f, 'steps_cost':cost, 
            'path':parent['path'], 'parent':parent} 
    
def heuristic_sld(map, goal, action):
    """Using Pythagorean theorem to compute Hsld"""
    a = abs(map.intersections[goal][0] - map.intersections[action][0])
    b = abs(map.intersections[goal][1] - map.intersections[action][1])
    return math.sqrt(a**2 + b**2)

def steps_cost(map, node, action):    
    """Using Pythagorean theorem for C(start, state)"""
    a = abs(map.intersections[node['state']][0] - map.intersections[action][0])
    b = abs(map.intersections[node['state']][1] - map.intersections[action][1])
    return node['steps_cost'] + math.sqrt(a**2 + b**2)        